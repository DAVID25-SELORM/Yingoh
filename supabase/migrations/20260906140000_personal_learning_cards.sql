begin;
alter table public.learning_profiles add column cv jsonb not null default '{}'::jsonb check(jsonb_typeof(cv)='object');
insert into public.learning_taxonomy(kind,code,label) values('program','career','International Nurse Career Hub');
create table public.personal_learning_cards (
