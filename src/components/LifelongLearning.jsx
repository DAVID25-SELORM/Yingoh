...
export default function LifelongLearning({ session, view = 'Learning Hub', onNavigate }) {
  ...
  return <section className="content-band lifelong">
    ...
    {view === 'Learning Hub' && <>
      ...
      <form className="lf-panel lf-form" onSubmit={e => { e.preventDefault(); act(() => request(db => db.from('learning_profiles').upsert({ ...profile, user_id: session.user.id, country_id: profile.country_id || null, specialty_id: profile.specialty_id || null, cycle_start: profile.cycle_start || null, cycle_end: profile.cycle_end || null, target_hours: Number(profile.target_hours), target_points: Number(profile.target_points) })), 'Learning preferences saved.'); }}>
        ...
        <label className="lf-check"><input type="checkbox" checked={profile.daily_email} onChange={e => setProfile({ ...profile, daily_email: e.target.checked })} /> Email me the daily learning question</label><button disabled={busy} className="primary-btn">Save preferences</button>
      </form>
